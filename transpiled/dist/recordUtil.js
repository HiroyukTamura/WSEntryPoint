...
var ERR_MSG_DUPLICATE_VAL = "項目名が重複しています";
var ERR_MSG_EACH_VAL_SAME = "項目名は開始と終了で別にしてください";
var ERR_MSG_NO_CONTENTS = '項目は最低でもひとつ必要です';

var TIME_POPOVER_CONFIG = {
    trigger: 'manual',
    placement: 'bottom',
    content: '<div class="circle-popover">' + '<span class="fa-layers modal-circle-w modal-circle-w1">' + '<i class="fas fa-circle modal-circle-i circle1"></i>' + '<i class="fas fa-check modal-circle-check" data-fa-transform="shrink-8"></i>' + '</span>' + '<span class="fa-layers modal-circle-w">' + '<i class="fas fa-circle modal-circle-i circle2"></i>' + '<i class="fas fa-check modal-circle-check" data-fa-transform="shrink-8"></i>' + '</span>' + '<span class="fa-layers modal-circle-w">' + '<i class="fas fa-circle modal-circle-i circle3"></i>' + '<i class="fas fa-check modal-circle-check" data-fa-transform="shrink-8"></i>' + '</span>' + '<span class="fa-layers modal-circle-w">' + '<i class="fas fa-circle modal-circle-i circle4"></i>' + '<i class="fas fa-check modal-circle-check" data-fa-transform="shrink-8"></i>' + '</span>' + '</div>',
    html: true,
    container: 'body',
    offset: '0, 15px'
};
...
function createAssEveRow(customClass, dataOrder) {
    ...
    return $('<tr class="add-eve-row ' + customClass + '" data-order="' + dataOrder + '">' + '<td colspan="4">' + '<button class="mdl-button mdl-js-button mdl-button--icon mdl-pre-upgrade" data-toggle="tooltip" data-placement="top" title="' + title + '">' + '<i class="material-icons">add_circle</i>' + '</button>' + '</tr>' + '</td>');
}

function createHtmlAs1Eve() {
    var random = Math.random().toString(36).slice(-8);
    return $('<tr>' + '<td class="circle_wrapper">' + '<div class="circle"></div>' + '</td>' + '<td>' + '<form action="#" class="time colored">' + '<div class="mdl-textfield mdl-js-textfield mdl-pre-upgrade mdl-textfield--floating-label">' + '<input class="mdl-textfield__input time_input mdl-pre-upgrade" type="text" id="sample">' + '<label class="mdl-textfield__label mdl-pre-upgrade" for="sample"></label>' + '<span class="mdl-textfield__error mdl-pre-upgrade"></span>' + '</div>' + '</form>' + '</td>' + '<td>' + '<form action="#" class="event_name">' + '<div class="mdl-textfield mdl-js-textfield mdl-pre-upgrade">' + '<input class="mdl-textfield__input input_eve mdl-pre-upgrade" type="text" id="' + random + '">' + '<label class="mdl-textfield__label mdl-pre-upgrade" for="' + random + '"></label>' + '<span class="mdl-textfield__error mdl-pre-upgrade"></span>' + '</div>' + '</form>' + '</td>' + '<td>' + '<button class="mdl-button mdl-js-button mdl-button--icon mdl-pre-upgrade remove-btn" data-toggle="tooltip" data-placement="top" title="項目を削除">' + '<i class="fas fa-times"></i>' + '</button>' + '</td>' + '</tr>')[0];
}
...
function createOneEveRow(doc, value, masterJson, saveBtn) {
    ...
    block.find('.circle').css('background-color', colors[value["colorNum"]]).popover(TIME_POPOVER_CONFIG).hover(function (e) {
        onHoverForPopover(e);
        ...
        popover.find('.fa-layers').on('click', function (e2) {
        ...
        }).eq(jsonC["eventList"][index]['colorNum']).find('.fa-check').show();
        ...
        $(window).hover(function (e2) {
            ...
            if ($(e2.target).parents('.popover').length || $(e2.target).hasClass('popover')) {
                return false;
            } else if (($(e2.target).hasClass('card') || $(e2.target).parents('tr').index() === index || $(e2.target).parents('tr').index() === index + 1) && ($(e2.target).parents('.card-wrapper-i[data-order=' + dataOrder + ']').length || $(e2.target).attr('data-order') == dataOrder)) {
                return false;
            }
    ...
    setDatePickerLisntener($(block).find('.time .mdl-textfield__input')).on('change', function (event, date) {
    ...
    block.find('.remove-btn').on('click', function (ev) {
        ...
        $(doc).find('table').find('tr').each(function (i, elem) {
            ...
            console.log(i, ym);
            if (ym && parseInt(ym) < 10000) {
                //range-preのdataOrderは10000
                ...
                for (var key in jsonC['eventList']) {
                    if (!jsonC['eventList'].hasOwnProperty(key) || key == index) continue;
...
function onHoverForPopover(e) {
    if (!$('.popover').length) $(e.target).popover('show');
}
...
function createOneRangeRow(doc, count, value, masterJson) {
    ...
    startInput.keyup(function (e) {
        ...
        for (var key in jsonC['rangeList']) {
            if (!jsonC['rangeList'].hasOwnProperty(key)) continue;

            if (key == index) continue;

            if (jsonC['rangeList'][key]['start']['name'] === $(e.target).val() && jsonC['rangeList'][key]['end']['name'] === endInput.val()) {
    ...
    endInput.keyup(function (e) {
        ...
        for (var key in jsonC['rangeList']) {
            ...
            if (jsonC['rangeList'][key]['start']['name'] === startInput.val() && jsonC['rangeList'][key]['end']['name'] === $(e.target).val()) {
    ...
    for (var i = 0; i < blocks.length; i++) {
        ...
        $(blocks[i]).find('.circle,.icon_down').popover(TIME_POPOVER_CONFIG).hover(function (e) {
            onHoverForPopover(e);
        }).on('shown.bs.popover', function (e) {
            console.log('onShown');
            // var dataOrder = $(this).parents(".card-wrapper-i").attr('data-order');
            ...
            popover.find('.fa-circle').on('click', function (e2) {
                console.log('cicle clicked');
                ...
                var trs = $(e.target).parents('tbody').find('tr[data-order="' + currentDataOrder + '"]');
...
function createHtmlAs1Row() {
    return $('<tr class="angle-down">' + '<td colspan="3">' + '<i class="fas fa-angle-double-down icon_down"></i>' + '</td>' + '<td colspan="1">' + '<button class="mdl-button mdl-js-button mdl-button--icon mdl-pre-upgrade remove-btn" data-toggle="tooltip" data-placement="top" title="項目を削除">' + '<i class="fas fa-times"></i>' + '</button>' + '</td>' + '</tr>')[0];
}

function createHtmlAs2() {
    return $('<div class="tag_wrapper" data-tag-order="0">' + '<span class="mdl-chip mdl-chip--deletable">' + '<span class="mdl-chip__text"></span>' + '<i class="fas fa-check fa-2x mdl-chip__action"></i>' + '<button type="button" class="mdl-chip__action"><i class="material-icons">cancel</i></button>' + '</span>' + '</div>');
}

function setRangeDatePicker(block, masterJson, offset) {
    var input = block.find('input').eq(0);
    setDatePickerLisntener(input).on('change', function (event, date) {
        var tr = $(event.target).parents('tr');
        ...
        var offset = $('.dtp').eq(jsonC["eventList"].length + dataNum * 2 + n).find('.date-picker-radios input:checked').parent().index() - 1;
        ...
        if (startOrEnd === 'start' && max === time || startOrEnd === 'end' && max === timeOther) {
            ...
            timeOtherInput.addClass('is-invalid').addClass('wrong-val').removeClass('is-focused').tooltip({
                title: '開始時刻が終了時刻を超えないようにしてください'
            });
            toggleBtn(false);
        } else {
...
function createHtmlRadio() {
    ...
    return $('<form class="date-picker-radios">' + '<tr>' + '<td>' + '<label class="mdl-radio mdl-js-radio mdl-js-ripple-effect mdl-pre-upgrade" for="' + id0 + '">' + '<input type="radio" id="' + id0 + '" class="mdl-radio__button mdl-pre-upgrade" name="options" value="1">' + '<span class="mdl-radio__label mdl-pre-upgrade">前日</span>' + '</label>' + '</td>' + '<td>' + '<label class="mdl-radio mdl-js-radio mdl-js-ripple-effect mdl-pre-upgrade" for="' + id1 + '">' + '<input type="radio" id="' + id1 + '" class="mdl-radio__button mdl-pre-upgrade" name="options" value="1">' + '<span class="mdl-radio__label mdl-pre-upgrade">当日</span>' + '</label>' + '</td>' + '<td>' + '<label class="mdl-radio mdl-js-radio mdl-js-ripple-effect mdl-pre-upgrade" for="' + id2 + '">' + '<input type="radio" id="' + id2 + '" class="mdl-radio__button mdl-pre-upgrade" name="options" value="1">' + '<span class="mdl-radio__label mdl-pre-upgrade">翌日</span>' + '</label>' + '</td>' + '</tr>' + '</form>');
}
...
function operateAs4(doc, childSnap, masterJson) {
    ...
    if (summery) clone.find(".mdl-textfield__input").attr("value", summery);
...
function createHtmlAs4() {
    return $('<div>' + '<div id="comment_dummy">' + '<form action="#" class="comment_form">' + '<div class="mdl-textfield mdl-js-textfield comment mdl-pre-upgrade">' + '<textarea class="mdl-textfield__input comment_ta mdl-pre-upgrade" type="text" rows= "3" id="comment" ></textarea>' + '<label class="mdl-textfield__label mdl-pre-upgrade" for="comment">Text...</label>' + '<span class="mdl-textfield__error mdl-pre-upgrade"></span>' + '</div>' + '</form>' + '</div>' + '</div>');
}
...
function createAddTagBtn() {
    return $('<button class="mdl-button mdl-js-button mdl-button--icon tag-add-btn mdl-pre-upgrade" data-tag-order="1000" data-toggle="tooltip" data-placement="top" title="項目を追加">' + '<i class="material-icons">add_circle</i>' + '</button>');
}
...
function onClickAddParamsLiBtn(splited, e, e2) {
    ...
    createParamsLi(splited, currentDataOrder, ul.length).appendTo(ul);
    setElementAsMdl(ul);
    $(e.target).popover('hide');
}

function createAddLiBtn() {
    return $('<div class="add-li-btn">' + '<button class="mdl-button mdl-js-button mdl-button--icon add-li-btn mdl-pre-upgrade" data-toggle="popover">' + '<i class="material-icons">add_circle</i>' + '</button>' + '</div>');
}
...
function initAddLiBtn() {
    ...
    addLiBtn.find('button').popover({
        trigger: 'manual',
        placement: 'bottom',
        content: '<div class="param-add-popover">' + '<button type="button" class="btn btn-secondary add-checkbox">チェックボックス</button>' + '<button type="button" class="btn btn-secondary add-slider">スライダー</button>' + '</div>',
        html: true,
        container: 'body',
        offset: '0 8px'
    }).hover(function (e) {
        ...
        poppover.find('.add-slider').on('click', function (e2) {
            var splited = ['1', '新しい項目', '3', '5'];
            onClickAddParamsLiBtn(splited, e, e2);
        });
        poppover.find('.add-checkbox').on('click', function (e2) {
...
//     console.log(masterJson);
// }
//# sourceMappingURL=recordUtil.js.map