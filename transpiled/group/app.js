...
/////////dialogまわり////////
var dialog = $('dialog')[0];
var dialogAddSch = $('#add-sch-cnt');
var dialogExclude = $('#dialog-img-w');
var dialogRemoveContents = $('#remove-contents');
var dialogEditComment = $('#edit-comment');
var dialogConfigGroup = $('#group-config');
var dialogAddFile = $('#add-file-modal');
var dialogNewDoc = $('#modal-new-doc');
var dialogShareRec = $('#share-rec');
var dialogPositibeBtn = $('#add-group-btn');
var dialogInvite = $('#invite-group');
var dialogAddDocComment = $('#add-doc-comment');
var clickedColor = 0;
var inputVal = null;
...
!function () {
    ...
    Calendar.prototype.drawDay = function (day) {
        ...
        outer.addEventListener('click', function () {
            if (!$(outer).hasClass('other')) self.openDay(this);
        });
    ...
    Calendar.prototype.drawEvents = function (day, element) {
        if (day.month() === this.current.month()) {
            ...
            for (var scheduleKey in dailyData) {
                if (!dailyData.hasOwnProperty(scheduleKey)) continue;
    ...
    Calendar.prototype.createChips = function (innerText, color) {
        ...
        ele.innerHTML = '<span class="mdl-chip__text mdl-pre-upgrade mdl-color-text--white">' + innerText + '</span>' + '<a href="#" class="mdl-chip__action mdl-pre-upgrade"><i class="material-icons">cancel</i></a>';
        return ele;
    };
    ...
    dialogPositibeBtn.on("click", function (e) {
        ...
        if (isModalOpen === dialogAddSch) {
            ...
            defaultDatabase.ref('group/' + groupKey + '/contents/' + uploadingData.contentKey).update(uploadingData).then(function (value) {
                ...
                showNotification('更新しました', 'success');
                if (!groupJson.contents) groupJson.contents = {};
    ...
    menus.eq(1).on('click', function (e) {
        ...
        if (groupJson.contents) {
            for (var contentKey in groupJson.contents) {
                if (groupJson.contents.hasOwnProperty(contentKey)) if (groupJson['contents'][contentKey]['type'] === 'data' && groupJson['contents'][contentKey]['whose'] === user.uid) {
                    key = contentKey;
                    break;
                }
            }
        ...
        } else showNotification(ERR_MSG_OPE_FAILED, 'danger');
    ...
    $('#member_conf').on('click', function (e) {
        ...
        for (var friendKey in friendJson) {
            ...
            for (var key in groupJson['member']) {
                if (!groupJson['member'].hasOwnProperty(key) || key === user.uid) continue;
                if (friendKey === key) {
                    isAdded = true;
                    break;
                }
...
function setOnGroupImageInputChange() {
    $('#group-config input').on('change', function (e) {
        ...
        if (groupImageTask || !e.target.files || !e.target.files[0]) return;
        ...
        console.log(suf);
        groupImageTask = firebase.storage().ref().child('group_icon').child(key + '.' + suf).put(e.target.files[0]);
...
function setFileModalListeners() {
    ...
    if (isAdvancedUpload) {
        dropDiv.on('drag dragstart dragend dragover dragenter dragleave drop', function (e) {
            ...
            if (!uploadingData && !uploadTask) dropDiv.addClass('is-dragover');
...
function onNewFileInputChange(file) {
    ...
    if (uploadTask || !file || uploadingData) return;

    var mimeType = file.type.toLowerCase();
    if (mimeType !== 'image/jpeg' && mimeType !== 'image/png' && mimeType !== 'image/gif' && mimeType !== "text/plain" && mimeType !== "text/txt" && mimeType !== "text/html" && mimeType !== "text/css" && mimeType !== "text/xml" && mimeType !== "application/pdf") {
    ...
    var extension = fileName.substring(dotPos + 1);
    if (extension !== 'jpeg' && extension !== 'jpg' && extension !== 'png' && extension !== 'gif' && extension !== "txt" && extension !== "html" && extension !== "css" && extension !== "xml" && extension !== "pdf") {
        showNotification('そのファイル形式はアップロードできません', 'warning');
    ...
    uploadTask = firebase.storage().ref().child('shareFile/' + groupKey).child(key + '.' + suf).put(file);
...
function addSchedule() {
    ...
    defaultDatabase.ref(makeRefScheme([scheme, eventKey])).update(newData).then(function () {
        if (!calendar.events[ym]) calendar.events[ym] = {};
        if (!calendar.events[ym][calendar.openedDay.date()]) calendar.events[ym][calendar.openedDay.date()] = {};
...
function initUserList() {
    ...
    keys.forEach(function (key) {
        if (key === DEFAULT || key === user.uid) return;
...
function createUserLi(key, friends) {
    ...
    var li = $('<li class="mdl-list__item mdl-pre-upgrade" key="' + key + '">' + '<span class="mdl-list__item-primary-content mdl-pre-upgrade">' + '<img src="' + photoUrl + '" alt="user-image">' + '<span>' + memberName + '&nbsp;&nbsp;' +
    // '<span class="config"><i class="fas fa-cog" id='+ key +'></i></span>' +
    '<div class="btn-group">' + '<button type="button" class="btn btn-secondary dropdown-toggle rounded-circle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' + '<i class="fas fa-cog"></i>' + '</button>' + '<div class="dropdown-menu dropdown-menu-right">' + '<button class="dropdown-item stop-share" type="button">体調記録のシェアをやめる</button>' + '<button class="dropdown-item reg-my-user" type="button">自分のユーザリストに登録する</button>' + '<button class="dropdown-item discourage" type="button">退会させる</button>' +
    // '<button class="dropdown-item cancel-inv" type="button">招待をとりやめる</button>'+
    '</div>' + '</div>' +
    // '</span>'+
    '</span>' + '</span>' + '<span class="mdl-list__item-secondary-action">' + '<span class="mdl-chip health-rec-btn">' + '<span class="mdl-chip__text">体調記録&nbsp;&nbsp;<i class="fas fa-external-link-square-alt fa-lg"></i></span>' + '</span>' + '</span>' + '</li>');

    //退会・招待とりやめ項目のセッティング
    var isAdded = groupJson["member"][key]['isChecked'];
    if (!isAdded) li.find('.discourage').hide();
    ...
    if (isFriend) li.find('.reg-my-user').hide();
    ...
    if (!isAdded && !isSharedRecord && isFriend) li.find('.dropdown-menu').hide();

    //chipsのセッティング
    if (!member.isChecked) {
        //todo isCheckedなのか、isAddedなのか
...
function initContents() {
    if (groupJson.contents) for (var key in groupJson.contents) {
        if (groupJson.contents.hasOwnProperty(key)) addOneContent(key);
    }
}
...
function appendContentAsDoc(contentData, key) {
    ...
    for (var i = 0; i < json.eleList.length; i++) {
        ...
        if (i === 0) {
            ...
            // setAsMyComment(cardCont, userUid);
            if (userUid === user.uid) cardCont.find('comment-follow').addClass('author-right');
...
(function () {
    function ContentAppenderForImg(contentData, key, timeline) {
        ...
        var comment = commentPre ? commentPre.replace(/(?:\r\n|\r|\n)/g, "<br />") : null;
        ...
        firebase.storage().ref("shareFile").child(groupKey).child(this.mKey + '.' + suf).getDownloadURL().then(function (url) {
            ...
            var errIcon = $('<span class="mdl-list__item-secondary-content mdl-pre-upgrade">' + '<i class="material-icons error-icon" title="画像を取得できませんでした">sms_failed</i>' + '</span>');
...
(function () {
    function ContentAppenderAsOtherFile(contentData, key, timeline) {
        ...
        firebase.storage().ref("shareFile/" + groupKey + "/" + this.mKey).getDownloadURL().then(function (url) {
            var openFileIcon = $('<span class="mdl-list__item-secondary-content mdl-pre-upgrade">' + '<span class="mdl-chip mdl-pre-upgrade show-file">' + '<span class="mdl-chip__text mdl-pre-upgrade">ファイルを表示&nbsp;&nbsp;<i class="fas fa-external-link-square-alt fa-lg"></i></span>' + '</span>' + '</span>');
            ...
            var errIcon = $('<span class="mdl-list__item-secondary-content mdl-pre-upgrade">' + '<i class="material-icons" title="ファイルを取得できませんでした">sms_failed</i>' + '</span>');
...
function createHtmlAsData(header, title, comment, key, photoUrl, fileUrl) {
    var ele = $('<div class="card file" key="' + key + '">' + '<div class="ele_header">' + '<img class="user-icon" src="' + photoUrl + '" alt="user-icon">' + '<span class="event_title">' + header + '</span>' + '<div class="mdl-layout-spacer mdl-pre-upgrade"></div>' + '<button class="mdl-button mdl-js-button mdl-button--icon edit-comment ele_header_button mdl-pre-upgrade" title="コメントを編集">' + '<i class="fas fa-comment"></i>' + '</button>' + '<button class="mdl-button mdl-js-button mdl-button--icon remove_btn ele_header_button mdl-pre-upgrade" title="コンテンツを削除">' + '<i class="fas fa-times"></i>' + '</button>' +
    ...
    // '</button>'+
    '</div>' + '<div class="flex-box file-wrapper">' + '<img class="file-icon" src="' + fileUrl + '" alt="file-icon">' + '<ul class="mdl-list mdl-pre-upgrade">' + '<li class="mdl-list__item mdl-list__item--two-line mdl-pre-upgrade">' + '<span class="mdl-list__item-primary-content mdl-pre-upgrade">' + '<span>' + title + '</span>' + '<span class="mdl-list__item-sub-title mdl-pre-upgrade">' + comment + '</span>' + '</span>' +
    ...
    //     '</button>'+
    // '</span>'+
    '</li>' + '</ul>' + '</div>' + '<div class="show-img">' + '<img alt="file-icon">' + '</div>' + '</div>');
...
function createHtmlAsDoc(title, ymd, whose, comment, photoUrl) {
    return $('<div class="card comments">' + '<div class="ele_header">' + '<i class="fas fa-comments fa-lg ele-icon"></i>' + '<span class="event_title">' + title + '</span>' + '<div class="mdl-layout-spacer"></div>' + '<button class="mdl-button mdl-js-button mdl-button--icon remove_btn ele_header_button mdl-pre-upgrade" title="コンテンツを削除">' + '<i class="fas fa-times"></i>' + '</button>' +
    // '<button class="mdl-button mdl-js-button mdl-button--icon arrow_up ele_header_button">'+
    //     '<i class="fas fa-angle-up">'+'</i>'+
    // '</button>'+
    '</div>' + '<hr class="seem">' + '<div class="card-cont">' + '<div class="comment-first space-horizontal">' + '<ul class="demo-list-three mdl-list">' + '<li class="mdl-list__item mdl-list__item--two-line mdl-pre-upgrade">' + '<span class="mdl-list__item-primary-content mdl-pre-upgrade">' + '<img src="' + photoUrl + '" alt="userA" class="mdl-list__item-icon mdl-pre-upgrade">' + '<span>' + whose + '</span>' + '<span class="mdl-list__item-sub-title mdl-pre-upgrade">' + ymd + '</span>' + '</span>' + '</li>' + '</ul>' + '<div class="comment-cont">' + comment + '</div>' + '</div>' + '<hr class="seem add-comment-prev">' + '<div class="add-comment">' + '<button class="mdl-button mdl-js-button mdl-button--icon mdl-pre-upgrade" title="コメントを追加">' + '<i class="material-icons">add_circle</i>' + '</button>' + '</div>' +
    // '<hr class="seem">'+
...
function createHtmlAsDocFollower(userName, ymd, comment, userPhotoUrl) {
    return $('<div>' + '<hr class="seem">' + '<div class="comment-follow">' + '<header class="flex-box">' + '<img src="' + userPhotoUrl + '" alt="userA" class="mdl-list__item-icon author-left">' + '<div>' + '<p class="comment-author">' + userName + '</p>' + '<p class="comment-time">' + ymd + '</p>' + '</div>' + '</header>' + '<div class="comment-cont">' + comment + '</div>' + '</div>');
}
...
function openDialog(toShowEle, fileName) {
    ...
    if (toShowEle === dialogAddSch) {
        ...
        for (var friendKey in friendJson) {
            ...
            $('<li class="mdl-list__item mdl-pre-upgrade" key="' + friendKey + '">' + '<span class="mdl-list__item-primary-content mdl-pre-upgrade">' + '<img src="' + photoUrl + '" class="mdl-list__item-avatar mdl-pre-upgrade">' + '<span>' + friendName + '</span>' + '</span>' + '<div class="mdl-layout-spacer"></div>' + '<div class="mdl-list__item-secondary-action mdl-pre-upgrade">' + '<label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect mdl-pre-upgrade" for="' + friendKey + '">' + '<input type="checkbox" id="' + friendKey + '" class="mdl-checkbox__input mdl-pre-upgrade">' + '</label>' + '</div>' + '</li>').appendTo(ul);
        }
...
function onSnapShotVanished() {
    if (!initialErr) showOpeErrNotification(defaultDatabase, -1);
    initialErr = true;
}
    ...
    Object.keys(groupNodeJson).forEach(function (key) {
        ...
        if (key === groupKey) {
            ...
            li.on('click', function (ev) {
                ...
                window.location.href = 'http://wppsc.php.xdomain.jp/public/group/index.html?key=' + liGroupKeys[index]; //todo これから
                return false;
            });
